...
/**
 * DELETE /api/expert/portfolios/[id]
 * Wipe all data for this expert portfolio so the user can re-initialize.
 */
export async function DELETE(
  ...
  try {
    ...
    const { data: portfolio } = await (supabase.from('expert_portfolio') as any)
      .select('id')
      .eq('id', id)
      .eq('user_id', user.id)
      .single();
    ...
    // Delete in FK order
    await (supabase.from('expert_transaction') as any).delete().eq('portfolio_id', id);
    await (supabase.from('expert_holding') as any).delete().eq('portfolio_id', id);
    await (supabase.from('expert_portfolio_snapshot') as any).delete().eq('portfolio_id', id);
    await (supabase.from('expert_portfolio') as any).delete().eq('id', id);

    return NextResponse.json({ success: true });
  } catch (err) {
    console.error('Expert portfolio DELETE error:', err);
