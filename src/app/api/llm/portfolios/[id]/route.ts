...
/**
 * DELETE /api/llm/portfolios/[id]
 * Wipe all data for this LLM portfolio so the user can re-initialize.
 */
export async function DELETE(
  ...
  try {
    ...
    const { data: portfolio } = await (supabase.from('llm_portfolio') as any)
      .select('id')
      .eq('id', id)
      .eq('user_id', user.id)
      .single();
    ...
    // Delete in FK order
    await (supabase.from('llm_transaction') as any).delete().eq('portfolio_id', id);
    await (supabase.from('llm_holding') as any).delete().eq('portfolio_id', id);
    await (supabase.from('llm_portfolio_snapshot') as any).delete().eq('portfolio_id', id);
    await (supabase.from('llm_portfolio') as any).delete().eq('id', id);

    return NextResponse.json({ success: true });
  } catch (err) {
    console.error('LLM portfolio DELETE error:', err);
